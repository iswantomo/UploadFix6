...
foreach($kolom_judul as $data){
	$i++;
	$objPHPExcel->setActiveSheetIndex()->setCellValue($array_cellkolom[$i]."1", $data);
}
...
foreach ($dataProvider->models as $data):
	$i++;
	$z=0;
	$z++; $objPHPExcel->setActiveSheetIndex()->setCellValue($array_cellkolom[$z].$i, ($i-1));
...
$objPHPExcel->setActiveSheetIndex()->getStyle('A1:'.$cell_terakhir)->applyFromArray($style_borderhitam);
$objPHPExcel->setActiveSheetIndex()->getStyle('A1:'.$cell_kolomterakhir.'1')->applyFromArray($style_tebal);
